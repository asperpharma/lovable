...
const CSV_FILE = 'c:/Users/C-R/Desktop/Asper Beauty Box/Asper Beauty shop prodcuts/product apify/FINAL_Asper_Catalog_82k.csv';
const BATCH_SIZE = 50;
...
const CATEGORY_KEYWORDS = {
    "skin-care": ["cleanser", "toner", "serum", "moisturizer", "cream", "face", "facial", "skin"],
    "hair-care": ["hair", "shampoo", "conditioner", "treatment", "oil", "mask", "scalp"],
    "make-up": ["mascara", "lipstick", "foundation", "eyeshadow", "blush", "concealer"],
...
async function run() {
    ...
    if (testError) {
        ...
        if (testData && testData.length > 0) {
            console.log('Existing columns found:', Object.keys(testData[0]).join(', '));
        }
    }
    ...
    for (let i = 0; i < products.length; i += BATCH_SIZE) {
        ...
        if (error) {
            ...
            process.stdout.write(`\rProgress: ${successCount} / ${products.length} products upserted...`);
        }
    }

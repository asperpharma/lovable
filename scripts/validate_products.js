...
const CSV_PATH = "c:/Users/C-R/Desktop/Asper Beauty Box/Asper Beauty shop prodcuts/product apify/1-3999 pro (1).csv";
...
function validateCSV() {
    ...
    const bodyIdx = headers.findIndex(h => h.toLowerCase().includes("body (html)") || h.toLowerCase() === "body");
    const typeIdx = headers.findIndex(h => h.toLowerCase() === "type");
    const tagsIdx = headers.findIndex(h => h.toLowerCase() === "tags");

    const imageIndices = headers.reduce((acc, h, i) => {
        if (h.toLowerCase().includes("image src") || h.toLowerCase().includes("images/0/src")) {
            acc.push(i);
        }
        return acc;
    }, []);
    ...
    const priceIndices = headers.reduce((acc, h, i) => {
        if (h.toLowerCase().includes("variant price") || h.toLowerCase().includes("variants/0/price")) {
            acc.push(i);
        }
        return acc;
    }, []);
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        for (const idx of imageIndices) {
            ...
            if (src && src.startsWith("http")) {
                hasValidImage = true;
                if (src.includes("thumbnail") || src.includes("_small") || src.includes("100x100")) {
                    issues.push(`[ROW ${rowIndex}] QUALITY: Low-res image for "${productTitle}"`);
                }
            }
        }
        if (!hasValidImage) {
            issues.push(`[ROW ${rowIndex}] MISSING IMAGE: No valid image URL for "${productTitle}"`);
        }
        ...
        if (!hasPrice) {
            issues.push(`[ROW ${rowIndex}] PRICE: Missing or zero price for "${productTitle}"`);
        }
        ...
        if (tags.toLowerCase().includes("organic") || type.toLowerCase().includes("organic")) {
            if (!body || body.length < 50) {
                issues.push(`[ROW ${rowIndex}] CONTENT: Organic product "${productTitle}" missing ingredients/info`);
            }
        }
    }

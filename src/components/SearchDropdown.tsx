...
export const SearchDropdown = ({
  ...
  return (
    <div
      ref={dropdownRef}
      className={`absolute ${isMobile ? 'left-0 right-0 top-full mt-1' : 'left-0 right-0 top-full mt-2'} bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden z-50 max-h-[400px] overflow-y-auto`}
    >
      {isLoading ? (
              ...
              return (
                <li key={product.node.id}>
                  <Link
                    ...
                    className={`flex items-center gap-4 px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0 ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
                    {/* Product Image */}
        ...
        <div className="py-8 text-center">
          ...
          <p className="text-gray-400 font-body text-xs mt-1">
            {language === 'ar' ? 'جربي كلمات بحث مختلفة' : 'Try different search terms'}
          </p>
        </div>
      )}
    </div>

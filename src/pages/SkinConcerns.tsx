...
export default function SkinConcerns() {
  ...
  return <div className="min-h-screen bg-soft-ivory" dir={isRTL ? "rtl" : "ltr"}>
      <Header />
      ...
      <main className="pt-32 lg:pt-40">
          ...
          <div className="luxury-container relative z-10">
            <div className="text-center max-w-3xl mx-auto">
              ...
              <p className="font-body text-lg text-gray-600 max-w-2xl mx-auto">
                {language === "ar" ? "اكتشفي المنتجات المثالية التي تستهدف احتياجات بشرتك الخاصة" : "Discover the perfect products that target your specific skin needs"}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 luxury-container">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            return <button key={concern.id} onClick={() => setSelectedConcern(isActive ? null : concern.id)} className={`group relative overflow-hidden rounded-2xl transition-all duration-500 ${isActive ? "ring-4 ring-shiny-gold shadow-xl scale-[1.02]" : "hover:shadow-lg hover:scale-[1.01]"}`}>
                  <div className="aspect-[4/3] relative">
                    <img src={concern.image} alt={language === "ar" ? concern.nameAr : concern.nameEn} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    ...
                    {isActive && <div className="absolute top-4 right-4 w-6 h-6 bg-shiny-gold rounded-full flex items-center justify-center">
        ...
        {selectedConcern && <section className="py-12 bg-white">
            <div className="luxury-container">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="font-display text-2xl md:text-3xl text-dark-charcoal">
                    {language === "ar" ? `منتجات ${activeConcern?.nameAr}` : `${activeConcern?.nameEn} Products`}
                  </h2>
                  <p className="font-body text-gray-600 mt-2">
                    {language === "ar" ? `${filteredProducts.length} منتج` : `${filteredProducts.length} products found`}
                  </p>
                </div>
                <button onClick={() => setSelectedConcern(null)} className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-dark-charcoal border border-gray-300 rounded-full hover:border-shiny-gold transition-colors">
              ...
              {loading ? <div className="flex items-center justify-center py-16">
                ...
                </div> : filteredProducts.length > 0 ? <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                  ...
                  <Link to="/collections/skin-care" className="inline-block mt-4 px-6 py-3 bg-shiny-gold text-black font-medium rounded-full hover:bg-shiny-gold/90 transition-colors">
        ...
        {/* CTA Section */}
        {!selectedConcern && <section className="py-16 bg-gradient-to-r from-dark-charcoal to-gray-800">
            <div className="luxury-container text-center">
              ...
              <Link to="/collections/skin-care" className="inline-block px-8 py-4 bg-shiny-gold text-black font-display tracking-wide rounded-full hover:bg-white transition-colors">

...
const PRODUCT_RANGES = [
  ...
  { 
    ...
    icon: Sparkles,
    color: 'bg-amber-500',
    keywords: ['liftactiv', 'lift activ', 'collagen', 'retinol', 'vitamin c', 'h.a.']
  },
  { 
    id: 'normaderm', 
    name: 'Normaderm', 
  ...
  const rangeCounts = useMemo(() => {
    ...
    PRODUCT_RANGES.forEach(range => {
      ...
      counts[range.id] = products.filter(product => {
        const title = product.node.title.toLowerCase();
        return range.keywords!.some(keyword => title.includes(keyword.toLowerCase()));
      }).length;
    });
                ...
                return (
                  ...
                  >
                    ...
                    <Badge 
                      ...
                      className={`
                        ml-1 text-xs
                        ${isActive ? 'bg-background/20 text-background' : 'bg-gold/10 text-gold'}
                      `}
                    >
                      {count}
                    </Badge>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <h2 className="font-display text-3xl md:text-4xl text-cream mb-6">
                  {isAr ? (
                    <>أكثر من <span className="text-gold">90 عاماً</span> من الابتكار</>
                  ) : (
                    <>Over <span className="text-gold">90 Years</span> of Innovation</>
                  )}
                </h2>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-4 bg-secondary/50 rounded-lg">
                    <div className="font-display text-2xl text-gold mb-1">15</div>
                    <div className="text-cream/60 text-sm">{isAr ? 'معدن' : 'Minerals'}</div>
                  </div>
                  <div className="text-center p-4 bg-secondary/50 rounded-lg">
                    <div className="font-display text-2xl text-gold mb-1">90+</div>
                    <div className="text-cream/60 text-sm">{isAr ? 'سنة' : 'Years'}</div>
                  </div>
                  <div className="text-center p-4 bg-secondary/50 rounded-lg">
                    <div className="font-display text-2xl text-gold mb-1">100%</div>
                    <div className="text-cream/60 text-sm">{isAr ? 'مختبر' : 'Tested'}</div>
                  </div>
                </div>
              </div>
